  ...
  namespace container
  {
    struct TimeSurfaceObservation
    {
      ...
      TimeSurfaceObservation(
          cv_bridge::CvImagePtr &left,
          cv_bridge::CvImagePtr &right,
          cv_bridge::CvImagePtr &last_now_left,
          size_t id,
          bool bCalcTsGradient = false)
          : id_(id)
      {
        cvImagePtr_left_ = left;
        cvImagePtr_right_ = right;
        cvImagePtr_last_now_left_ = last_now_left;
        cv::cv2eigen(left->image, TS_left_);
        cv::cv2eigen(right->image, TS_right_);
        cv::cv2eigen(last_now_left->image, last_now_left_);
      ...
      inline bool isLastEmpty()
      {
        if (TS_last_.rows() == 0 || TS_last_.cols() == 0 || (tr_last_.getPosition() - tr_.getPosition()).norm() < 0.1)
          return true;
        else
          return false;
      }
      ...
      Eigen::MatrixXd TS_left_, TS_right_, TS_last_, last_now_left_;
      Eigen::MatrixXd TS_blurred_left_;
      Eigen::MatrixXd TS_negative_left_;
      cv_bridge::CvImagePtr cvImagePtr_left_, cvImagePtr_right_, cvImagePtr_last_, cvImagePtr_last_now_left_;
      Transformation tr_, tr_last_;
