...
esvo_core::core::EventMatcher::EventMatcher(
    ...
    double patch_valid_ratio) : camSysPtr_(camSysPtr),
    ...
    EventMatchPair &emPair)
{
  ...
  for (size_t i = 0; i < candidates_epipolar_check.size(); i++)
  {
    ...
    if (warping2(x_left, 1.0 / depth, T_left_rv.block<3, 4>(0, 0), x1_s, x2_s))
    {
      ...
      if (patchInterpolation2(pTS_obs_->second.TS_left_, x1_s, patch_left) && patchInterpolation2(pTS_obs_->second.TS_right_, x2_s, patch_right))
      {
...
bool esvo_core::core::EventMatcher::patchInterpolation2(
