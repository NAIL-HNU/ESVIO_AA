...
namespace esvo_core
{
  namespace core
  {
    DepthProblem::DepthProblem(
        ...
        const CameraSystem::Ptr &camSysPtr) : optimization::OptimizationFunctor<double>(1, 0),
    ...
    int DepthProblem::operator()(const Eigen::VectorXd &x, Eigen::VectorXd &fvec) const
    {
      ...
      if (patchInterpolation(pStampedTsObs_->second.TS_left_, x1_s, tau1) && patchInterpolation(pStampedTsObs_->second.TS_right_, x2_s, tau2))
      {
