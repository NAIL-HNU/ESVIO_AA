...
namespace esvo_core
{
  namespace core
  {
    DepthFusion::DepthFusion(
        ...
        std::shared_ptr<DepthProblemConfig> &dpConfigPtr) : camSysPtr_(camSysPtr),
                                                            dpConfigPtr_(dpConfigPtr) {}

    DepthFusion::~DepthFusion() {}

    bool DepthFusion::propagate_one_point(
        DepthPoint &dp_prior,
        DepthPoint &dp_prop,
    ...
    bool DepthFusion::boundaryCheck(
        double xcoor,
        double ycoor,
        size_t width,
        size_t height)
    ...
    bool DepthFusion::chiSquareTest(
    ...
    bool DepthFusion::studentTCompatibleTest(
        double invD1, double invD2,
        double var1, double var2)
    {
    ...
    void DepthFusion::naive_propagation(
        std::vector<DepthPoint> &dp_obs,
        DepthFrame::Ptr &df)
    {
      Eigen::Matrix<double, 4, 4> T_frame_world = df->T_world_frame_.inverse().getTransformationMatrix();
    ...
    bool DepthFusion::naive_propagate_one_point(
        DepthPoint &dp_prior,
        DepthPoint &dp_prop,
        Eigen::Matrix<double, 4, 4> &T_prop_prior)
    {
